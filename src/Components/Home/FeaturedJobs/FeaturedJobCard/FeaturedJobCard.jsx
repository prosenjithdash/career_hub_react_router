...
const FeaturedJobCard = ({ job }) => {
    ...
    return (
        <div className="border border-gray-200 rounded-lg p-8 space-y-2">
            <div>
                <img className="w-[116.686px] rounded-lg h-[40px]" src={logo} alt="" />
            </div>
            <h3 className="text-[24px]">{job_title}</h3>
            <h3 className="text-[#757575] text-[20px]">{company_name}</h3>
            <div className="flex gap-5">
                <button className="btn border border-[#7E90FE] py-2 px-4 bg-white">Remote</button>
                <button className="btn border border-[#7E90FE] bg-white py-2 px-4">Full Time</button>
            </div>
            <div className="flex gap-5">
            ...
            <Link to={`/job/${id} `}><button className="bg-[#1631e0] rounded-lg text-white py-4 px-4 font-bold">View Details</button></Link>
